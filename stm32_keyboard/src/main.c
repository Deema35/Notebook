...
KeyDesk* KeyRows[8][12] = 
{
  ...
  {&Key_Q, &Key_3, &Key_semicolon, &Enter, &Key_I, &Key_Y, &Key_H, &Key_Tab, &PgUp, &Key_E, &Key_S, NULL, NULL},
...
int main(void)
{
  ...
  while (1)
  {
    ...
    for (int i = 0; i < 8; i++)
    {
      ...
      for (int j = 0; j < 13; j++)
      {
        if (KeyRows[i][j])
        {
          ...
          if (!CurentState)
          {
             if (!KeyNum && KeyRows[i][j]->KEYCODE1)
            {
              HIDkeyBoardMessage.KEYCODE1 = KeyRows[i][j]->KEYCODE1;
              KeyNum = 1;
            }
    ...
    if(KeyNum || KeyMod)
    {
      USBD_HID_SendReport(&hUsbDeviceFS,&HIDkeyBoardMessage,sizeof(HIDkeyBoardMessage));
      IsKeyPressed = 1;
